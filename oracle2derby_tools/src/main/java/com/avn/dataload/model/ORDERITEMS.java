package com.avn.dataload.model;

import java.math.BigDecimal;
import java.util.Date;

public class ORDERITEMS {
    

    private java.math.BigDecimal ORDERITEMS_ID;
    
    public void setORDERITEMS_ID(java.math.BigDecimal ORDERITEMS_ID) {
        this.ORDERITEMS_ID = ORDERITEMS_ID;
    }
    
    public java.math.BigDecimal getORDERITEMS_ID() {
        return this.ORDERITEMS_ID;
    }
      

    private java.math.BigDecimal STOREENT_ID;
    
    public void setSTOREENT_ID(java.math.BigDecimal STOREENT_ID) {
        this.STOREENT_ID = STOREENT_ID;
    }
    
    public java.math.BigDecimal getSTOREENT_ID() {
        return this.STOREENT_ID;
    }
      

    private java.math.BigDecimal ORDERS_ID;
    
    public void setORDERS_ID(java.math.BigDecimal ORDERS_ID) {
        this.ORDERS_ID = ORDERS_ID;
    }
    
    public java.math.BigDecimal getORDERS_ID() {
        return this.ORDERS_ID;
    }
      

    private java.math.BigDecimal TERMCOND_ID;
    
    public void setTERMCOND_ID(java.math.BigDecimal TERMCOND_ID) {
        this.TERMCOND_ID = TERMCOND_ID;
    }
    
    public java.math.BigDecimal getTERMCOND_ID() {
        return this.TERMCOND_ID;
    }
      

    private java.math.BigDecimal TRADING_ID;
    
    public void setTRADING_ID(java.math.BigDecimal TRADING_ID) {
        this.TRADING_ID = TRADING_ID;
    }
    
    public java.math.BigDecimal getTRADING_ID() {
        return this.TRADING_ID;
    }
      

    private java.math.BigDecimal ITEMSPC_ID;
    
    public void setITEMSPC_ID(java.math.BigDecimal ITEMSPC_ID) {
        this.ITEMSPC_ID = ITEMSPC_ID;
    }
    
    public java.math.BigDecimal getITEMSPC_ID() {
        return this.ITEMSPC_ID;
    }
      

    private java.math.BigDecimal CATENTRY_ID;
    
    public void setCATENTRY_ID(java.math.BigDecimal CATENTRY_ID) {
        this.CATENTRY_ID = CATENTRY_ID;
    }
    
    public java.math.BigDecimal getCATENTRY_ID() {
        return this.CATENTRY_ID;
    }
      

    private java.lang.String PARTNUM;
    
    public void setPARTNUM(java.lang.String PARTNUM) {
        this.PARTNUM = PARTNUM;
    }
    
    public java.lang.String getPARTNUM() {
        return this.PARTNUM;
    }
      

    private java.math.BigDecimal SHIPMODE_ID;
    
    public void setSHIPMODE_ID(java.math.BigDecimal SHIPMODE_ID) {
        this.SHIPMODE_ID = SHIPMODE_ID;
    }
    
    public java.math.BigDecimal getSHIPMODE_ID() {
        return this.SHIPMODE_ID;
    }
      

    private java.math.BigDecimal FFMCENTER_ID;
    
    public void setFFMCENTER_ID(java.math.BigDecimal FFMCENTER_ID) {
        this.FFMCENTER_ID = FFMCENTER_ID;
    }
    
    public java.math.BigDecimal getFFMCENTER_ID() {
        return this.FFMCENTER_ID;
    }
      

    private java.math.BigDecimal MEMBER_ID;
    
    public void setMEMBER_ID(java.math.BigDecimal MEMBER_ID) {
        this.MEMBER_ID = MEMBER_ID;
    }
    
    public java.math.BigDecimal getMEMBER_ID() {
        return this.MEMBER_ID;
    }
      

    private java.math.BigDecimal ADDRESS_ID;
    
    public void setADDRESS_ID(java.math.BigDecimal ADDRESS_ID) {
        this.ADDRESS_ID = ADDRESS_ID;
    }
    
    public java.math.BigDecimal getADDRESS_ID() {
        return this.ADDRESS_ID;
    }
      

    private java.math.BigDecimal ALLOCADDRESS_ID;
    
    public void setALLOCADDRESS_ID(java.math.BigDecimal ALLOCADDRESS_ID) {
        this.ALLOCADDRESS_ID = ALLOCADDRESS_ID;
    }
    
    public java.math.BigDecimal getALLOCADDRESS_ID() {
        return this.ALLOCADDRESS_ID;
    }
      

    private java.math.BigDecimal PRICE;
    
    public void setPRICE(java.math.BigDecimal PRICE) {
        this.PRICE = PRICE;
    }
    
    public java.math.BigDecimal getPRICE() {
        return this.PRICE;
    }
      

    private java.lang.String LINEITEMTYPE;
    
    public void setLINEITEMTYPE(java.lang.String LINEITEMTYPE) {
        this.LINEITEMTYPE = LINEITEMTYPE;
    }
    
    public java.lang.String getLINEITEMTYPE() {
        return this.LINEITEMTYPE;
    }
      

    private java.lang.String STATUS;
    
    public void setSTATUS(java.lang.String STATUS) {
        this.STATUS = STATUS;
    }
    
    public java.lang.String getSTATUS() {
        return this.STATUS;
    }
      

    private java.math.BigDecimal OUTPUTQ_ID;
    
    public void setOUTPUTQ_ID(java.math.BigDecimal OUTPUTQ_ID) {
        this.OUTPUTQ_ID = OUTPUTQ_ID;
    }
    
    public java.math.BigDecimal getOUTPUTQ_ID() {
        return this.OUTPUTQ_ID;
    }
      

    private java.lang.String INVENTORYSTATUS;
    
    public void setINVENTORYSTATUS(java.lang.String INVENTORYSTATUS) {
        this.INVENTORYSTATUS = INVENTORYSTATUS;
    }
    
    public java.lang.String getINVENTORYSTATUS() {
        return this.INVENTORYSTATUS;
    }
      

    private java.sql.Timestamp LASTCREATE;
    
    public void setLASTCREATE(java.sql.Timestamp LASTCREATE) {
        this.LASTCREATE = LASTCREATE;
    }
    
    public java.sql.Timestamp getLASTCREATE() {
        return this.LASTCREATE;
    }
      

    private java.sql.Timestamp LASTUPDATE;
    
    public void setLASTUPDATE(java.sql.Timestamp LASTUPDATE) {
        this.LASTUPDATE = LASTUPDATE;
    }
    
    public java.sql.Timestamp getLASTUPDATE() {
        return this.LASTUPDATE;
    }
      

    private java.lang.String FULFILLMENTSTATUS;
    
    public void setFULFILLMENTSTATUS(java.lang.String FULFILLMENTSTATUS) {
        this.FULFILLMENTSTATUS = FULFILLMENTSTATUS;
    }
    
    public java.lang.String getFULFILLMENTSTATUS() {
        return this.FULFILLMENTSTATUS;
    }
      

    private java.sql.Timestamp LASTALLOCUPDATE;
    
    public void setLASTALLOCUPDATE(java.sql.Timestamp LASTALLOCUPDATE) {
        this.LASTALLOCUPDATE = LASTALLOCUPDATE;
    }
    
    public java.sql.Timestamp getLASTALLOCUPDATE() {
        return this.LASTALLOCUPDATE;
    }
      

    private java.math.BigDecimal OFFER_ID;
    
    public void setOFFER_ID(java.math.BigDecimal OFFER_ID) {
        this.OFFER_ID = OFFER_ID;
    }
    
    public java.math.BigDecimal getOFFER_ID() {
        return this.OFFER_ID;
    }
      

    private java.sql.Timestamp TIMERELEASED;
    
    public void setTIMERELEASED(java.sql.Timestamp TIMERELEASED) {
        this.TIMERELEASED = TIMERELEASED;
    }
    
    public java.sql.Timestamp getTIMERELEASED() {
        return this.TIMERELEASED;
    }
      

    private java.sql.Timestamp TIMESHIPPED;
    
    public void setTIMESHIPPED(java.sql.Timestamp TIMESHIPPED) {
        this.TIMESHIPPED = TIMESHIPPED;
    }
    
    public java.sql.Timestamp getTIMESHIPPED() {
        return this.TIMESHIPPED;
    }
      

    private java.lang.String CURRENCY;
    
    public void setCURRENCY(java.lang.String CURRENCY) {
        this.CURRENCY = CURRENCY;
    }
    
    public java.lang.String getCURRENCY() {
        return this.CURRENCY;
    }
      

    private java.lang.String COMMENTS;
    
    public void setCOMMENTS(java.lang.String COMMENTS) {
        this.COMMENTS = COMMENTS;
    }
    
    public java.lang.String getCOMMENTS() {
        return this.COMMENTS;
    }
      

    private java.math.BigDecimal TOTALPRODUCT;
    
    public void setTOTALPRODUCT(java.math.BigDecimal TOTALPRODUCT) {
        this.TOTALPRODUCT = TOTALPRODUCT;
    }
    
    public java.math.BigDecimal getTOTALPRODUCT() {
        return this.TOTALPRODUCT;
    }
      

    private java.math.BigDecimal QUANTITY;
    
    public void setQUANTITY(java.math.BigDecimal QUANTITY) {
        this.QUANTITY = QUANTITY;
    }
    
    public java.math.BigDecimal getQUANTITY() {
        return this.QUANTITY;
    }
      

    private java.math.BigDecimal TAXAMOUNT;
    
    public void setTAXAMOUNT(java.math.BigDecimal TAXAMOUNT) {
        this.TAXAMOUNT = TAXAMOUNT;
    }
    
    public java.math.BigDecimal getTAXAMOUNT() {
        return this.TAXAMOUNT;
    }
      

    private java.math.BigDecimal TOTALADJUSTMENT;
    
    public void setTOTALADJUSTMENT(java.math.BigDecimal TOTALADJUSTMENT) {
        this.TOTALADJUSTMENT = TOTALADJUSTMENT;
    }
    
    public java.math.BigDecimal getTOTALADJUSTMENT() {
        return this.TOTALADJUSTMENT;
    }
      

    private java.math.BigDecimal SHIPTAXAMOUNT;
    
    public void setSHIPTAXAMOUNT(java.math.BigDecimal SHIPTAXAMOUNT) {
        this.SHIPTAXAMOUNT = SHIPTAXAMOUNT;
    }
    
    public java.math.BigDecimal getSHIPTAXAMOUNT() {
        return this.SHIPTAXAMOUNT;
    }
      

    private java.sql.Timestamp ESTAVAILTIME;
    
    public void setESTAVAILTIME(java.sql.Timestamp ESTAVAILTIME) {
        this.ESTAVAILTIME = ESTAVAILTIME;
    }
    
    public java.sql.Timestamp getESTAVAILTIME() {
        return this.ESTAVAILTIME;
    }
      

    private java.math.BigDecimal FIELD1;
    
    public void setFIELD1(java.math.BigDecimal FIELD1) {
        this.FIELD1 = FIELD1;
    }
    
    public java.math.BigDecimal getFIELD1() {
        return this.FIELD1;
    }
      

    private java.lang.String DESCRIPTION;
    
    public void setDESCRIPTION(java.lang.String DESCRIPTION) {
        this.DESCRIPTION = DESCRIPTION;
    }
    
    public java.lang.String getDESCRIPTION() {
        return this.DESCRIPTION;
    }
      

    private java.lang.String FIELD2;
    
    public void setFIELD2(java.lang.String FIELD2) {
        this.FIELD2 = FIELD2;
    }
    
    public java.lang.String getFIELD2() {
        return this.FIELD2;
    }
      

    private java.math.BigDecimal ALLOCATIONGROUP;
    
    public void setALLOCATIONGROUP(java.math.BigDecimal ALLOCATIONGROUP) {
        this.ALLOCATIONGROUP = ALLOCATIONGROUP;
    }
    
    public java.math.BigDecimal getALLOCATIONGROUP() {
        return this.ALLOCATIONGROUP;
    }
      

    private java.math.BigDecimal SHIPCHARGE;
    
    public void setSHIPCHARGE(java.math.BigDecimal SHIPCHARGE) {
        this.SHIPCHARGE = SHIPCHARGE;
    }
    
    public java.math.BigDecimal getSHIPCHARGE() {
        return this.SHIPCHARGE;
    }
      

    private java.math.BigDecimal BASEPRICE;
    
    public void setBASEPRICE(java.math.BigDecimal BASEPRICE) {
        this.BASEPRICE = BASEPRICE;
    }
    
    public java.math.BigDecimal getBASEPRICE() {
        return this.BASEPRICE;
    }
      

    private java.lang.String BASECURRENCY;
    
    public void setBASECURRENCY(java.lang.String BASECURRENCY) {
        this.BASECURRENCY = BASECURRENCY;
    }
    
    public java.lang.String getBASECURRENCY() {
        return this.BASECURRENCY;
    }
      

    private java.lang.String TRACKNUMBER;
    
    public void setTRACKNUMBER(java.lang.String TRACKNUMBER) {
        this.TRACKNUMBER = TRACKNUMBER;
    }
    
    public java.lang.String getTRACKNUMBER() {
        return this.TRACKNUMBER;
    }
      

    private java.sql.Timestamp TRACKDATE;
    
    public void setTRACKDATE(java.sql.Timestamp TRACKDATE) {
        this.TRACKDATE = TRACKDATE;
    }
    
    public java.sql.Timestamp getTRACKDATE() {
        return this.TRACKDATE;
    }
      

    private java.math.BigDecimal PREPAREFLAGS;
    
    public void setPREPAREFLAGS(java.math.BigDecimal PREPAREFLAGS) {
        this.PREPAREFLAGS = PREPAREFLAGS;
    }
    
    public java.math.BigDecimal getPREPAREFLAGS() {
        return this.PREPAREFLAGS;
    }
      

    private java.math.BigDecimal CORRELATIONGROUP;
    
    public void setCORRELATIONGROUP(java.math.BigDecimal CORRELATIONGROUP) {
        this.CORRELATIONGROUP = CORRELATIONGROUP;
    }
    
    public java.math.BigDecimal getCORRELATIONGROUP() {
        return this.CORRELATIONGROUP;
    }
      

    private java.sql.Timestamp PROMISEDAVAILTIME;
    
    public void setPROMISEDAVAILTIME(java.sql.Timestamp PROMISEDAVAILTIME) {
        this.PROMISEDAVAILTIME = PROMISEDAVAILTIME;
    }
    
    public java.sql.Timestamp getPROMISEDAVAILTIME() {
        return this.PROMISEDAVAILTIME;
    }
      

    private java.math.BigDecimal SHIPPINGOFFSET;
    
    public void setSHIPPINGOFFSET(java.math.BigDecimal SHIPPINGOFFSET) {
        this.SHIPPINGOFFSET = SHIPPINGOFFSET;
    }
    
    public java.math.BigDecimal getSHIPPINGOFFSET() {
        return this.SHIPPINGOFFSET;
    }
      

    private java.math.BigDecimal NEEDEDQUANTITY;
    
    public void setNEEDEDQUANTITY(java.math.BigDecimal NEEDEDQUANTITY) {
        this.NEEDEDQUANTITY = NEEDEDQUANTITY;
    }
    
    public java.math.BigDecimal getNEEDEDQUANTITY() {
        return this.NEEDEDQUANTITY;
    }
      

    private java.math.BigDecimal ALLOCQUANTITY;
    
    public void setALLOCQUANTITY(java.math.BigDecimal ALLOCQUANTITY) {
        this.ALLOCQUANTITY = ALLOCQUANTITY;
    }
    
    public java.math.BigDecimal getALLOCQUANTITY() {
        return this.ALLOCQUANTITY;
    }
      

    private java.math.BigDecimal ALLOCFFMC_ID;
    
    public void setALLOCFFMC_ID(java.math.BigDecimal ALLOCFFMC_ID) {
        this.ALLOCFFMC_ID = ALLOCFFMC_ID;
    }
    
    public java.math.BigDecimal getALLOCFFMC_ID() {
        return this.ALLOCFFMC_ID;
    }
      

    private java.math.BigDecimal ORDRELEASENUM;
    
    public void setORDRELEASENUM(java.math.BigDecimal ORDRELEASENUM) {
        this.ORDRELEASENUM = ORDRELEASENUM;
    }
    
    public java.math.BigDecimal getORDRELEASENUM() {
        return this.ORDRELEASENUM;
    }
      

    private java.lang.String CONFIGURATIONID;
    
    public void setCONFIGURATIONID(java.lang.String CONFIGURATIONID) {
        this.CONFIGURATIONID = CONFIGURATIONID;
    }
    
    public java.lang.String getCONFIGURATIONID() {
        return this.CONFIGURATIONID;
    }
      

    private java.lang.String SUPPLIERDATA;
    
    public void setSUPPLIERDATA(java.lang.String SUPPLIERDATA) {
        this.SUPPLIERDATA = SUPPLIERDATA;
    }
    
    public java.lang.String getSUPPLIERDATA() {
        return this.SUPPLIERDATA;
    }
      

    private java.lang.String SUPPLIERPARTNUMBER;
    
    public void setSUPPLIERPARTNUMBER(java.lang.String SUPPLIERPARTNUMBER) {
        this.SUPPLIERPARTNUMBER = SUPPLIERPARTNUMBER;
    }
    
    public java.lang.String getSUPPLIERPARTNUMBER() {
        return this.SUPPLIERPARTNUMBER;
    }
      

    private java.math.BigDecimal AVAILQUANTITY;
    
    public void setAVAILQUANTITY(java.math.BigDecimal AVAILQUANTITY) {
        this.AVAILQUANTITY = AVAILQUANTITY;
    }
    
    public java.math.BigDecimal getAVAILQUANTITY() {
        return this.AVAILQUANTITY;
    }
      

    private java.lang.String ISEXPEDITED;
    
    public void setISEXPEDITED(java.lang.String ISEXPEDITED) {
        this.ISEXPEDITED = ISEXPEDITED;
    }
    
    public java.lang.String getISEXPEDITED() {
        return this.ISEXPEDITED;
    }
      

    private java.math.BigDecimal OPTCOUNTER;
    
    public void setOPTCOUNTER(java.math.BigDecimal OPTCOUNTER) {
        this.OPTCOUNTER = OPTCOUNTER;
    }
    
    public java.math.BigDecimal getOPTCOUNTER() {
        return this.OPTCOUNTER;
    }
      

    private java.sql.Timestamp REQUESTEDSHIPDATE;
    
    public void setREQUESTEDSHIPDATE(java.sql.Timestamp REQUESTEDSHIPDATE) {
        this.REQUESTEDSHIPDATE = REQUESTEDSHIPDATE;
    }
    
    public java.sql.Timestamp getREQUESTEDSHIPDATE() {
        return this.REQUESTEDSHIPDATE;
    }
      

    private java.math.BigDecimal TIECODE;
    
    public void setTIECODE(java.math.BigDecimal TIECODE) {
        this.TIECODE = TIECODE;
    }
    
    public java.math.BigDecimal getTIECODE() {
        return this.TIECODE;
    }
      
    
}