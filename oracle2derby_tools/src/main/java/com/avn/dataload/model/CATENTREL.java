package com.avn.dataload.model;

public class CATENTREL {


    private java.math.BigDecimal CATENTRY_ID_PARENT;
    
    public void setCATENTRY_ID_PARENT(java.math.BigDecimal CATENTRY_ID_PARENT) {
        this.CATENTRY_ID_PARENT = CATENTRY_ID_PARENT;
    }
    
    public java.math.BigDecimal getCATENTRY_ID_PARENT() {
        return this.CATENTRY_ID_PARENT;
    }
      

    private java.lang.String CATRELTYPE_ID;
    
    public void setCATRELTYPE_ID(java.lang.String CATRELTYPE_ID) {
        this.CATRELTYPE_ID = CATRELTYPE_ID;
    }
    
    public java.lang.String getCATRELTYPE_ID() {
        return this.CATRELTYPE_ID;
    }
      

    private java.math.BigDecimal CATENTRY_ID_CHILD;
    
    public void setCATENTRY_ID_CHILD(java.math.BigDecimal CATENTRY_ID_CHILD) {
        this.CATENTRY_ID_CHILD = CATENTRY_ID_CHILD;
    }
    
    public java.math.BigDecimal getCATENTRY_ID_CHILD() {
        return this.CATENTRY_ID_CHILD;
    }
      

    private java.math.BigDecimal SEQUENCE;
    
    public void setSEQUENCE(java.math.BigDecimal SEQUENCE) {
        this.SEQUENCE = SEQUENCE;
    }
    
    public java.math.BigDecimal getSEQUENCE() {
        return this.SEQUENCE;
    }
      

    private java.math.BigDecimal QUANTITY;
    
    public void setQUANTITY(java.math.BigDecimal QUANTITY) {
        this.QUANTITY = QUANTITY;
    }
    
    public java.math.BigDecimal getQUANTITY() {
        return this.QUANTITY;
    }
      

    private java.lang.String GROUPNAME;
    
    public void setGROUPNAME(java.lang.String GROUPNAME) {
        this.GROUPNAME = GROUPNAME;
    }
    
    public java.lang.String getGROUPNAME() {
        return this.GROUPNAME;
    }
      

    private java.lang.String FIELD1;
    
    public void setFIELD1(java.lang.String FIELD1) {
        this.FIELD1 = FIELD1;
    }
    
    public java.lang.String getFIELD1() {
        return this.FIELD1;
    }
      

    private java.math.BigDecimal FIELD2;
    
    public void setFIELD2(java.math.BigDecimal FIELD2) {
        this.FIELD2 = FIELD2;
    }
    
    public java.math.BigDecimal getFIELD2() {
        return this.FIELD2;
    }
      

    private java.math.BigDecimal FIELD3;
    
    public void setFIELD3(java.math.BigDecimal FIELD3) {
        this.FIELD3 = FIELD3;
    }
    
    public java.math.BigDecimal getFIELD3() {
        return this.FIELD3;
    }
      

    private java.lang.String OID;
    
    public void setOID(java.lang.String OID) {
        this.OID = OID;
    }
    
    public java.lang.String getOID() {
        return this.OID;
    }
      

    private java.math.BigDecimal OPTCOUNTER;
    
    public void setOPTCOUNTER(java.math.BigDecimal OPTCOUNTER) {
        this.OPTCOUNTER = OPTCOUNTER;
    }
    
    public java.math.BigDecimal getOPTCOUNTER() {
        return this.OPTCOUNTER;
    }
      

    private java.lang.String MANDATORY;
    
    public void setMANDATORY(java.lang.String MANDATORY) {
        this.MANDATORY = MANDATORY;
    }
    
    public java.lang.String getMANDATORY() {
        return this.MANDATORY;
    }
      

    public String toString() {
        StringBuilder result = new StringBuilder();
        result.append("CATENTRY_ID_PARENT: " + getCATENTRY_ID_PARENT() + "||");
        result.append("CATRELTYPE_ID: " + getCATRELTYPE_ID() + "||");
        result.append("CATENTRY_ID_CHILD: " + getCATENTRY_ID_CHILD() + "||");
        result.append("SEQUENCE: " + getSEQUENCE() + "||");
        result.append("QUANTITY: " + getQUANTITY() + "||");
        result.append("GROUPNAME: " + getGROUPNAME() + "||");
        result.append("FIELD1: " + getFIELD1() + "||");
        result.append("FIELD2: " + getFIELD2() + "||");
        result.append("FIELD3: " + getFIELD3() + "||");
        result.append("OID: " + getOID() + "||");
        result.append("OPTCOUNTER: " + getOPTCOUNTER() + "||");
        result.append("MANDATORY: " + getMANDATORY() + "||");
        return result.toString();
    }
    
}