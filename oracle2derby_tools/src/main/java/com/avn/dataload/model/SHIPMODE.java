package com.avn.dataload.model;

public class SHIPMODE {


    private java.math.BigDecimal SHIPMODE_ID;
    
    public void setSHIPMODE_ID(java.math.BigDecimal SHIPMODE_ID) {
        this.SHIPMODE_ID = SHIPMODE_ID;
    }
    
    public java.math.BigDecimal getSHIPMODE_ID() {
        return this.SHIPMODE_ID;
    }
      

    private java.lang.String FIELD1;
    
    public void setFIELD1(java.lang.String FIELD1) {
        this.FIELD1 = FIELD1;
    }
    
    public java.lang.String getFIELD1() {
        return this.FIELD1;
    }
      

    private java.math.BigDecimal STOREENT_ID;
    
    public void setSTOREENT_ID(java.math.BigDecimal STOREENT_ID) {
        this.STOREENT_ID = STOREENT_ID;
    }
    
    public java.math.BigDecimal getSTOREENT_ID() {
        return this.STOREENT_ID;
    }
      

    private java.math.BigDecimal FIELD2;
    
    public void setFIELD2(java.math.BigDecimal FIELD2) {
        this.FIELD2 = FIELD2;
    }
    
    public java.math.BigDecimal getFIELD2() {
        return this.FIELD2;
    }
      

    private java.lang.String CODE;
    
    public void setCODE(java.lang.String CODE) {
        this.CODE = CODE;
    }
    
    public java.lang.String getCODE() {
        return this.CODE;
    }
      

    private java.lang.String CARRIER;
    
    public void setCARRIER(java.lang.String CARRIER) {
        this.CARRIER = CARRIER;
    }
    
    public java.lang.String getCARRIER() {
        return this.CARRIER;
    }
      

    private java.lang.String TRACKINGNAME;
    
    public void setTRACKINGNAME(java.lang.String TRACKINGNAME) {
        this.TRACKINGNAME = TRACKINGNAME;
    }
    
    public java.lang.String getTRACKINGNAME() {
        return this.TRACKINGNAME;
    }
      

    private java.lang.String TRACKINGURL;
    
    public void setTRACKINGURL(java.lang.String TRACKINGURL) {
        this.TRACKINGURL = TRACKINGURL;
    }
    
    public java.lang.String getTRACKINGURL() {
        return this.TRACKINGURL;
    }
      

    private java.lang.String TRACKINGHOST;
    
    public void setTRACKINGHOST(java.lang.String TRACKINGHOST) {
        this.TRACKINGHOST = TRACKINGHOST;
    }
    
    public java.lang.String getTRACKINGHOST() {
        return this.TRACKINGHOST;
    }
      

    private java.math.BigDecimal TRACKINGPORT;
    
    public void setTRACKINGPORT(java.math.BigDecimal TRACKINGPORT) {
        this.TRACKINGPORT = TRACKINGPORT;
    }
    
    public java.math.BigDecimal getTRACKINGPORT() {
        return this.TRACKINGPORT;
    }
      

    private java.lang.String TRACKINGICON;
    
    public void setTRACKINGICON(java.lang.String TRACKINGICON) {
        this.TRACKINGICON = TRACKINGICON;
    }
    
    public java.lang.String getTRACKINGICON() {
        return this.TRACKINGICON;
    }
      

    private java.lang.String TRACKINGTYPE;
    
    public void setTRACKINGTYPE(java.lang.String TRACKINGTYPE) {
        this.TRACKINGTYPE = TRACKINGTYPE;
    }
    
    public java.lang.String getTRACKINGTYPE() {
        return this.TRACKINGTYPE;
    }
      

    private java.math.BigDecimal MARKFORDELETE;
    
    public void setMARKFORDELETE(java.math.BigDecimal MARKFORDELETE) {
        this.MARKFORDELETE = MARKFORDELETE;
    }
    
    public java.math.BigDecimal getMARKFORDELETE() {
        return this.MARKFORDELETE;
    }
      

    private java.math.BigDecimal OPTCOUNTER;
    
    public void setOPTCOUNTER(java.math.BigDecimal OPTCOUNTER) {
        this.OPTCOUNTER = OPTCOUNTER;
    }
    
    public java.math.BigDecimal getOPTCOUNTER() {
        return this.OPTCOUNTER;
    }
      

    public String toString() {
        StringBuilder result = new StringBuilder();
        result.append("SHIPMODE_ID: " + getSHIPMODE_ID() + "||");
        result.append("FIELD1: " + getFIELD1() + "||");
        result.append("STOREENT_ID: " + getSTOREENT_ID() + "||");
        result.append("FIELD2: " + getFIELD2() + "||");
        result.append("CODE: " + getCODE() + "||");
        result.append("CARRIER: " + getCARRIER() + "||");
        result.append("TRACKINGNAME: " + getTRACKINGNAME() + "||");
        result.append("TRACKINGURL: " + getTRACKINGURL() + "||");
        result.append("TRACKINGHOST: " + getTRACKINGHOST() + "||");
        result.append("TRACKINGPORT: " + getTRACKINGPORT() + "||");
        result.append("TRACKINGICON: " + getTRACKINGICON() + "||");
        result.append("TRACKINGTYPE: " + getTRACKINGTYPE() + "||");
        result.append("MARKFORDELETE: " + getMARKFORDELETE() + "||");
        result.append("OPTCOUNTER: " + getOPTCOUNTER() + "||");
        return result.toString();
    }
    
}