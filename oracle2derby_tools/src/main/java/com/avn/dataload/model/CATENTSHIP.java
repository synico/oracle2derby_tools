package com.avn.dataload.model;

public class CATENTSHIP {


    private java.math.BigDecimal CATENTRY_ID;
    
    public void setCATENTRY_ID(java.math.BigDecimal CATENTRY_ID) {
        this.CATENTRY_ID = CATENTRY_ID;
    }
    
    public java.math.BigDecimal getCATENTRY_ID() {
        return this.CATENTRY_ID;
    }
      

    private java.math.BigDecimal WEIGHT;
    
    public void setWEIGHT(java.math.BigDecimal WEIGHT) {
        this.WEIGHT = WEIGHT;
    }
    
    public java.math.BigDecimal getWEIGHT() {
        return this.WEIGHT;
    }
      

    private java.lang.String WEIGHTMEASURE;
    
    public void setWEIGHTMEASURE(java.lang.String WEIGHTMEASURE) {
        this.WEIGHTMEASURE = WEIGHTMEASURE;
    }
    
    public java.lang.String getWEIGHTMEASURE() {
        return this.WEIGHTMEASURE;
    }
      

    private java.math.BigDecimal LENGTH;
    
    public void setLENGTH(java.math.BigDecimal LENGTH) {
        this.LENGTH = LENGTH;
    }
    
    public java.math.BigDecimal getLENGTH() {
        return this.LENGTH;
    }
      

    private java.math.BigDecimal WIDTH;
    
    public void setWIDTH(java.math.BigDecimal WIDTH) {
        this.WIDTH = WIDTH;
    }
    
    public java.math.BigDecimal getWIDTH() {
        return this.WIDTH;
    }
      

    private java.math.BigDecimal HEIGHT;
    
    public void setHEIGHT(java.math.BigDecimal HEIGHT) {
        this.HEIGHT = HEIGHT;
    }
    
    public java.math.BigDecimal getHEIGHT() {
        return this.HEIGHT;
    }
      

    private java.lang.String SIZEMEASURE;
    
    public void setSIZEMEASURE(java.lang.String SIZEMEASURE) {
        this.SIZEMEASURE = SIZEMEASURE;
    }
    
    public java.lang.String getSIZEMEASURE() {
        return this.SIZEMEASURE;
    }
      

    private java.math.BigDecimal NOMINALQUANTITY;
    
    public void setNOMINALQUANTITY(java.math.BigDecimal NOMINALQUANTITY) {
        this.NOMINALQUANTITY = NOMINALQUANTITY;
    }
    
    public java.math.BigDecimal getNOMINALQUANTITY() {
        return this.NOMINALQUANTITY;
    }
      

    private java.math.BigDecimal QUANTITYMULTIPLE;
    
    public void setQUANTITYMULTIPLE(java.math.BigDecimal QUANTITYMULTIPLE) {
        this.QUANTITYMULTIPLE = QUANTITYMULTIPLE;
    }
    
    public java.math.BigDecimal getQUANTITYMULTIPLE() {
        return this.QUANTITYMULTIPLE;
    }
      

    private java.lang.String QUANTITYMEASURE;
    
    public void setQUANTITYMEASURE(java.lang.String QUANTITYMEASURE) {
        this.QUANTITYMEASURE = QUANTITYMEASURE;
    }
    
    public java.lang.String getQUANTITYMEASURE() {
        return this.QUANTITYMEASURE;
    }
      

    private java.math.BigDecimal OPTCOUNTER;
    
    public void setOPTCOUNTER(java.math.BigDecimal OPTCOUNTER) {
        this.OPTCOUNTER = OPTCOUNTER;
    }
    
    public java.math.BigDecimal getOPTCOUNTER() {
        return this.OPTCOUNTER;
    }
      

    public String toString() {
        StringBuilder result = new StringBuilder();
        result.append("CATENTRY_ID: " + getCATENTRY_ID() + "||");
        result.append("WEIGHT: " + getWEIGHT() + "||");
        result.append("WEIGHTMEASURE: " + getWEIGHTMEASURE() + "||");
        result.append("LENGTH: " + getLENGTH() + "||");
        result.append("WIDTH: " + getWIDTH() + "||");
        result.append("HEIGHT: " + getHEIGHT() + "||");
        result.append("SIZEMEASURE: " + getSIZEMEASURE() + "||");
        result.append("NOMINALQUANTITY: " + getNOMINALQUANTITY() + "||");
        result.append("QUANTITYMULTIPLE: " + getQUANTITYMULTIPLE() + "||");
        result.append("QUANTITYMEASURE: " + getQUANTITYMEASURE() + "||");
        result.append("OPTCOUNTER: " + getOPTCOUNTER() + "||");
        return result.toString();
    }
    
}